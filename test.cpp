...
public:
	...
	void remove_KOKUSEN()
	{
		for(int i = 1; i < MAXSIZE + 1; i++){//! BỎ hết khối luôn 
			string temp = areaTable[i].test_print();
...
public:
	...
	void remove_KEITEIKEN(int number)
	{
		...
		for(int i = 0; i < areaTable.size(); i++){ //! BỎ hết khối luôn
			Heap_before_KEITEIKEN.push_back({areaTable[i]->ID, areaTable[i]->size()}); 
			string temp = "ID=" + to_string(areaTable[i]->ID) + " : ";
			for(list<int>::iterator it = areaTable[i]->head.begin();it != areaTable[i]->head.end(); ++it)
			{
				temp += to_string(*it) + " ";
			}
		...
		for(int i = 0; i < areaTable.size(); i++){ //! BỎ hết khối luôn
			Heap_after_KEITEIKEN.push_back({areaTable[i]->ID, areaTable[i]->size()}); 
			string temp = "ID=" + to_string(areaTable[i]->ID) + " : ";
			for(list<int>::iterator it = areaTable[i]->head.begin();it != areaTable[i]->head.end(); ++it)
			{
				temp += to_string(*it) + " ";
